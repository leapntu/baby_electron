...
function preload () {
  ...
  reps = parseInt(meta['test_reps'])
  while (i < reps) {
    sim_test.push( test_words[ i % test_words.length ])
    i += 1
  }
  test_words = sim_test
...
function next_test_stimuli() {
  ...
  audio_obj = createjs.Sound.play("t"+parseInt(stimuli))
...
function mainLoop (delta) {
  if (mode == 'begin') {
    ...
    if (meta['training'] == 'n') {begin_message = "When ready to begin, tap SPACE" }
...
function handle_keydown (e) {
  if (space_down == false && e.code == "Space") {
    ...
    if (mode == 'skiptrain'){video_div.style.display = "inline"; canvas.style.display = "none"; start_getter()}
  }
}
